...
public class IsoGridPosition extends GridPosition {
  ...
  public int getX() {
    return (int) (((this.position % 64) - 0.5 + ((Math.floor(this.position / 64) % 2) / 2)) * IsoGridPosition
        .getIsoGridWidth()) + this.offsetX;
  }

  public int getY() {
    return (int) ((Math.floor(this.position / 64) + 1) * (IsoGridPosition.getIsoGridHeight() / 2))
        + this.offsetY;
  }
  ...
  protected static synchronized int getIsoGridWidth() {
    if (IsoGridPosition.isoGridWidth == null) {
      IsoGridPosition.isoGridWidth =
          GameSetting
              .getNumber(
                  ((XMLHandler) TSOExpert.getHandler("XMLHandler"))
                      .getDocument(GameSetting.gfx_settings),
                  "//Globals/IsoGrid/@w").intValue();
    }
    if (IsoGridPosition.isoGridWidth != null) {
  ...
  protected static synchronized int getIsoGridHeight() {
    if (IsoGridPosition.isoGridHeight == null) {
      IsoGridPosition.isoGridHeight =
          GameSetting
              .getNumber(
                  ((XMLHandler) TSOExpert.getHandler("XMLHandler"))
                      .getDocument(GameSetting.gfx_settings),
                  "//Globals/IsoGrid/@h").intValue();
    }
    if (IsoGridPosition.isoGridHeight != null) {
