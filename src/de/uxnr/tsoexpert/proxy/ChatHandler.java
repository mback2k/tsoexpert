      ...
      for (int x = 0; x < messages.getLength(); x++) {
        ...
        if (this.isTradeMessage(n)) {
          TradeRequest tradeRequest =
              new TradeRequest(n.getTextContent(), this.extractPlayerInformation(n));
  ...
  private boolean isTradeMessage(Node n) {
    ...
    for (int x = 0; x < attributes.getLength(); x++) {
      ...
      if (attribute.getNodeName().equalsIgnoreCase("type")
          && attribute.getNodeValue().equalsIgnoreCase("groupchat"))
        groupchat = true;
      if (attribute.getNodeName().equalsIgnoreCase("from")
          && attribute.getNodeValue().contains("trade@"))
        fromtrade = true;
    }
