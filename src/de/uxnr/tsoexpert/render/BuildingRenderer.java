...
public class BuildingRenderer extends AbstractRenderer {
  ...
  private int getAvailableLevel(String name, int level) {
    ...
    if (this.nofUpgrades.containsKey(name)) {
      ...
      nofUpgrades =
          GameSetting.getNumber(this.xmlHandler.getDocument(GameSetting.gfx_settings),
              "//GameObjects/Buildings/Building[@name='" + name + "']/@nofUpgrades");
  ...
  private Sprite getSprite(final Building building) {
    ...
    String filename =
        this.getSpriteFilename("//GameObjects/Buildings/Building[@name='" + name + "']/@filename",
            level, type);
