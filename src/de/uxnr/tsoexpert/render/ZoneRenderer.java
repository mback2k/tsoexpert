...
public class ZoneRenderer {
	public Rectangle renderZone(Graphics2D graphics, Rectangle clip) {
		...
		for (Entry<GridPosition, Object> entry : objects.entrySet()) {
			...
			if (object instanceof Background) {
				...
				this.renderFreeLandscape((FreeLandscape) object, freeLandscapeRenderer, graphics, clip, frame);
	...
	protected void renderBackground(Background background, BackgroundRenderer backgroundRenderer, Graphics2D graphics, Rectangle clip, Rectangle frame) {
		this.updateFrame(backgroundRenderer.renderBackground(background, graphics, clip), frame);
	}

	protected void renderFreeLandscape(FreeLandscape freeLandscape, FreeLandscapeRenderer freeLandscapeRenderer, Graphics2D graphics, Rectangle clip, Rectangle frame) {
		this.updateFrame(freeLandscapeRenderer.renderFreeLandscape(freeLandscape, graphics, clip), frame);
	}

	protected void renderLandscape(Landscape landscape, LandscapeRenderer landscapeRenderer, Graphics2D graphics, Rectangle clip, Rectangle frame) {
		this.updateFrame(landscapeRenderer.renderLandscape(landscape, graphics, clip), frame);
	}

	protected void renderBuilding(Building building, BuildingRenderer buildingRenderer, Graphics2D graphics, Rectangle clip, Rectangle frame) {
